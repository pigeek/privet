...
@pytest.mark.asyncio
async def test_chunk_node(debugger_server_port: int):
    project_path = fixture_path("nodes/chunk_node/graphs/happy.rivet-project")
    ...
    assert normalized == [
        ...
        {"message": "nodeStart", "node": {"id": "n2", "type": "chunk"}},
        {"message": "nodeFinish", "node": {"id": "n2", "type": "chunk"}},
    ...
    assert outputs["chunks"] == {"type": "string[]", "value": ["abcdefghijklmnopqrst", "uvwxyz"]}
    assert outputs["count"] == {"type": "number", "value": 2}
