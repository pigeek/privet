...
@pytest.mark.asyncio
async def test_destructure_node(debugger_server_port: int):
    project_path = fixture_path("nodes/destructure_node/graphs/happy.rivet-project")
    ...
    assert normalized == [
        ...
        {"message": "nodeStart", "node": {"id": "n2", "type": "destructure"}},
        {"message": "nodeFinish", "node": {"id": "n2", "type": "destructure"}},
    ...
    assert outputs["name"] == {"type": "any", "value": "Rivet"}
    assert outputs["city"] == {"type": "any", "value": "Austin"}
