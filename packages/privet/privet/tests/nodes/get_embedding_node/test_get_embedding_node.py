...
@pytest.mark.asyncio
async def test_get_embedding_node(debugger_server_port: int):
    project_path = fixture_path("nodes/get_embedding_node/graphs/happy.rivet-project")
    ...
    embedding = outputs["out"]
    assert embedding["type"] == "vector"
    assert len(embedding["value"]) == 3
