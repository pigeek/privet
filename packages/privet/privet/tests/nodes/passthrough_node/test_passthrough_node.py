...
@pytest.mark.asyncio
async def test_passthrough_node(debugger_server_port: int):
    project_path = fixture_path("nodes/passthrough_node/graphs/happy.rivet-project")
    project = load_rivet_project(project_path)
    events = await run_graph_via_ws(debugger_server_port, project, {"in": {"type": "string", "value": "hello"}})
    ...
    assert normalized == [
        ...
        {"message": "nodeStart", "node": {"id": "n2", "type": "passthroughNode"}},
        {"message": "nodeFinish", "node": {"id": "n2", "type": "passthroughNode"}},
    ...
    assert outputs["out"] == {"type": "string", "value": "hello"}
