...
@pytest.mark.asyncio
async def test_trim_chat_messages_node(debugger_server_port: int):
    project_path = fixture_path("nodes/trim_chat_messages_node/graphs/happy.rivet-project")
    ...
    assert normalized == [
        ...
        {"message": "nodeStart", "node": {"id": "n2", "type": "trimChatMessages"}},
        {"message": "nodeFinish", "node": {"id": "n2", "type": "trimChatMessages"}},
    ...
    trimmed = outputs["out"]
    assert trimmed["type"] == "chat-message[]"
    assert trimmed["value"] == [
        {
            "type": "assistant",
            "message": ["this is a very long message that should push us over the token limit"],
        }
    ]
