...
@pytest.mark.asyncio
async def test_append_to_dataset_node(debugger_server_port: int):
    project_path = fixture_path("nodes/append_to_dataset_node/graphs/happy.rivet-project")
    ...
    ds = outputs["dataset"]["value"]
    assert ds["id"] == "ds2"
    assert ds["rows"][0]["data"] == {"type": "string[]", "value": ["row"]}
