...
@pytest.mark.asyncio
async def test_split_node_basic(debugger_server_port: int):
    project_path = fixture_path("nodes/split_node/graphs/happy.rivet-project")
    project = load_rivet_project(project_path)
    events = await run_graph_via_ws(debugger_server_port, project, {"text": {"type": "string", "value": "a,b,c"}})
    ...
    assert outputs["out"] == {"type": "string[]", "value": ["a", "b", "c"]}
