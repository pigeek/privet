...
@pytest.mark.asyncio
async def test_graph_reference_node(debugger_server_port: int):
    project_path = fixture_path("nodes/graph_reference_node/graphs/happy.rivet-project")
    ...
    graph_ref = outputs["out"]
    assert graph_ref["type"] == "graph-reference"
    assert graph_ref["value"]["graphId"] == "sub"
